...
scores = cross_val_score(DT, attributes, label, cv=10, scoring='f1_macro',n_jobs=1)
#print(scores)
#print("F-score: %0.2f (+/= % 0.2f)" % (scores.mean(),scores.std()*2))

DT_Model = DT.fit(attributes,label)

feature_names = attributes.columns[:9]
fig = plt.figure(figsize=(500,2000))
